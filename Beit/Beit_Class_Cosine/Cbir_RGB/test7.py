...
class ScrollableImageGallery(tk.Frame):
    ...
    def load_images(self, folder):
        ...
        # update the scrollregion of the canvas to start from x=0 and end at the last image x
        last_row_widgets = self.scrollable_frame.grid_slaves(column=0, row=i//3+1)
        last_row_widget_right_edge = last_row_widgets[-1].winfo_x() + last_row_widgets[-1].winfo_width()
        self.canvas.configure(scrollregion=(0, 0, last_row_widget_right_edge, self.scrollable_frame.winfo_height()))
