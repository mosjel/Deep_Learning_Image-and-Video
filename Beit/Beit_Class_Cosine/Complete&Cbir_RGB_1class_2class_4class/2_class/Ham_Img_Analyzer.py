...
class featHA:
    ...
    def ham_img_Analyzer(self):
        ...
        for cou,address in enumerate(pics):
            ...
            class_1=np.argmax(class_)
            # class_=np.argpartition(class_,-40)[-40:]
            class_2=np.argpartition(class_,-2)[-2:]
            class_3=np.argpartition(class_,-3)[-3:]
            class_4=np.argpartition(class_,-4)[-4:]
            ...
            comb=[class_1,class_2,class_3,class_4,filename]
            comb.extend(out_[0])
            comb.extend(iml)
            out_1.append(comb)  
            if cou==10 :
